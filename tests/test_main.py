...
def test_add_duplicate_participant():
    ...
    game = SecretSanta()
    game.add_participant('testuser', 'test@example.com')
    game.add_participant('testuser', 'another@example.com') # Same name, different email
    game.add_participant('anotheruser', 'test@example.com') # Different name, same email
    assert len(game.participants) == 1
...
def test_assign_santas():
    ...
    for giver, receiver in game.assignments.items():
        ...
        assert receiver in [p['name'] for p in game.participants] # Receiver is a valid participant
